...
class Ordinary_Car:
    ...
    def traverse(self, nav, margin):
        if nav == "forward":
            self.set_motor_model(1885,2000,2000,2000)
            time.sleep(margin)
            self.set_motor_model(0,0,0,0)
        elif nav == "right":
            self.set_motor_model(2000,2000,-2000,-2000)
            time.sleep(margin)
            self.set_motor_model(0,0,0,0)
        elif nav == "left":
            self.set_motor_model(-2000,-2000,2000,2000)
...
if __name__ == '__main__':
    ...
    try:
        PWM.traverse("forward",1.8)
        PWM.traverse("right",.29)
        PWM.traverse("forward",1)
        PWM.traverse("right",.5)
        PWM.traverse("forward",.575)
        PWM.traverse("right",.773)
        PWM.traverse("forward", 2.3)
        PWM.traverse("left",.73)
        PWM.traverse("forward",.475)
        PWM.traverse("left",.6)
        PWM.traverse("forward",1.75)
